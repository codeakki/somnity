...
export interface TransactionRequest {
  fromWalletId: string;
  toAddress: string;
  amount: string; // in STT (will be converted to wei)
  gasLimit?: bigint;
}
...
export class TransactionService {
  ...
  private static shouldSponsorTransaction(gasCost: bigint, walletBalance: bigint, transactionValue: bigint): boolean {
    ...
    const gasCostInSTT = Number(formatEther(gasCost));
    
    // Sponsor if gas cost is more than 1 STT (very expensive)
    const shouldSponsor = gasCostInSTT > 1.0;
  ...
  private static async sponsorTransaction(walletAddress: string, gasCost: bigint): Promise<{ success: boolean; error?: string }> {
    try {
      const gasCostInSTT = formatEther(gasCost);
      console.log(`Sponsoring transaction for wallet ${walletAddress} with gas cost ${gasCostInSTT} STT`);
      
      const result = await FundingService.fundWalletForGas(walletAddress, gasCostInSTT);
  ...
  /**
   * Send STT tokens from wallet to another address
    ...
    try {
      ...
      try {
        ...
        if (serverBalanceWei < totalGasCost) {
          ...
          return {
            success: false,
            error: `Relayer has insufficient funds for gas. Required: ${needed} STT, Available: ${available} STT. Please top-up relayer or try again later.`,
          };
        }
      } catch (e) {
      ...
      if (shouldSponsor) {
        ...
        if (updatedBalance.balanceWei < amountWei) {
          return {
            success: false,
            error: `Insufficient balance even after sponsorship. Required: ${request.amount} STT for transfer, Available: ${updatedBalance.balance} STT`,
          };
        }
      } else {
        ...
        if (walletBalance.balanceWei < amountWei) {
          return {
            success: false,
            error: `Insufficient balance for transfer. Required: ${request.amount} STT, Available: ${walletBalance.balance} STT`,
          };
        }
      }
