...
export const SendTransaction = ({ wallet, isOpen, onClose, onTransactionSent }: SendTransactionProps) => {
    ...
    try {
      ...
      if (result.success && result.transactionHash) {
        const description = result.sponsored 
          ? `Successfully sent ${amount} STT to ${recipientDisplayName} (Gas fees sponsored)`
          : `Successfully sent ${amount} STT to ${recipientDisplayName}`;
  ...
  const formatGasCost = (gas: bigint) => {
    ...
    const gasInSTT = Number(gas) / 1e18 * 0.000000001; // Rough estimate
    return gasInSTT.toFixed(6);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Send STT tokens from {wallet.name}
          </DialogDescription>
        </DialogHeader>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">{walletBalance.balance} STT</span>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span>Gas Cost:</span>
                  <span>
                    {sponsorshipInfo ? sponsorshipInfo.gasCost : formatGasCost(gasEstimate!)} STT
      ...
      <PinModal
        ...
        description={
          pendingTransaction 
            ? `Send ${pendingTransaction.amount} STT to ${pendingTransaction.recipient}`
