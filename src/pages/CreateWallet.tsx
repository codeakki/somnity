...
const CreateWallet = () => {
  ...
  const [keyGenerationError, setKeyGenerationError] = useState<string | null>(
    null
  );
    ...
    if (
      ...
      const estimateGas = async () => {
        try {
          ...
          const gasEstimate = await estimateWalletCreationGas(
            walletName,
          ...
          console.log(
            "CreateWallet - Gas estimation complete in useEffect:",
            gasEstimate
          );
        } catch (error) {
  ...
  const handleRememberDeviceConfirm = async (
    pin: string,
    ...
    try {
      if (rememberDevice && walletKeys && createdWalletId) {
        ...
        await KeyManagementService.storeEncryptedKeys(
          createdWalletId,
          walletKeys,
          pin
        );
        toast({
          ...
          description:
            "Your wallet will be remembered on this device. You can change this setting later in wallet settings.",
        });
      }
      ...
      toast({
        title: "Error",
        description:
          "Failed to set up device storage. You can configure this later in wallet settings.",
        variant: "destructive",
      });
    ...
    if (step === 5) {
      ...
      try {
        ...
        if (!currentGasEstimate) {
          console.log(
            "CreateWallet - No gas estimate found, estimating now..."
          );
          setIsEstimatingGas(true);
          currentGasEstimate = await estimateWalletCreationGas(
            walletName,
            walletKeys.privateKey
          );
          ...
          setIsEstimatingGas(false);
          console.log(
            "CreateWallet - Gas estimation complete:",
            currentGasEstimate
          );
        } else {
          console.log(
            "CreateWallet - Using existing gas estimate:",
            currentGasEstimate
          );
        }
        ...
        setIsFunding(true);
        const fundingResult = await FundingService.fundWalletForGas(
          walletKeys.address,
          currentGasEstimate.gasCostInSTT
        );
        ...
        if (fundingResult.transactionHash === "already_funded") {
          toast({
            ...
            description: `Wallet already has sufficient funds (${(
              parseFloat(currentGasEstimate.gasCostInSTT) * 1.1
            ).toFixed(6)} STT) for gas fees`,
          });
        } else {
          toast({
            title: "Wallet Funded",
            description: `Wallet funded with ${(
              parseFloat(currentGasEstimate.gasCostInSTT) * 1.1
            ).toFixed(6)} STT for gas fees`,
          });
        }
        ...
        const contractAddress = await deploySmartContractWallet(
          walletName,
          walletKeys.privateKey
        );
        ...
        const addressExists = await walletService.checkAddressExists(
          contractAddress
        );
        if (addressExists) {
        ...
        if (savedWallet) {
          ...
          toast({
            ...
            description: `Wallet "${walletName}.somnia" (${contractAddress.slice(
              0,
              8
            )}...${contractAddress.slice(
              -6
            )}) has been created and deployed to the blockchain.`,
          });
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="flex justify-between mt-6">
          ...
          <Button
            ...
            disabled={
              !canContinue() || isSaving || isEstimatingGas || isFunding
            }
